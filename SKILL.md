...
## Phase 2c: 四层质检（content-quality-gate）

**执行时机**：报告草稿产出后、正式交付前。

对报告草稿执行 `content-quality-gate` 四层质检：

| 层级 | 检查内容 | 通过标准 |
|------|---------|---------|
| L1 硬性规则 | 禁用词、空话、事实硬伤、格式一致性 | 零容忍 |
| L2 可读性 | 开头、节奏、结构、读者视角 | L2-1 必须，其余 3/4 |
| L3 内容深度 | 论据支撑、独创性、对立面、可执行性 | L3-1/2 必须，其余 2/3 |
| L4 活人感 | 温度感、独特性、姿态、心流 | 整体"像真人写的" |

**质检不修改内容**，只输出质检报告。如有问题，明确标注段落位置，由主 session 修复后再交付。质检通过后进入 Phase 3。
...
2. **发送用户**：
   - 标题 + 摘要预览（前 5 行）
   - 全文
   - 附注：`已完成。报告共 X 字，Phase 0 命中 {N} 个本地文件，Phase 1 抓取 {Z} 个来源，Phase 2c 四层质检 [通过/需修复]，参考资料 {M} 篇已归档。`
3. **记录到记忆**：
